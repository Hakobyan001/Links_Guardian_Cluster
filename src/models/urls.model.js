...
class Data {
  ...
  static async getLinksAndUrls(campaignId) {
    ...
    if (!sendLinks[0].changeing || !sendUrls[0].changeing){
      ...
      for (let i = 0; i < sendUrls.length; i++) {
        externalInfo.push({
          ...
          rel: sendUrls[i].rel,
          keyword: sendUrls[i].keyword,
          robot_tag: sendUrls[i].robot_tag,
          status: sendUrls[i].changeing_status,
        ...
        const info = [];
          info.push({
            ...
            title: sendLinks[0].title,
            robot_tag: sendLinks[0].robot_tag,
            favicon: sendLinks[0].favicon,
            status: sendLinks[0].status,
    ...
    else if (sendLinks[0].changeing.length == 4) {
      for (let i = 0; i < sendUrls.length; i++) {
        externalInfo.push({
          id: sendUrls[i].id,
          external_urls: sendUrls[i].external_urls,
